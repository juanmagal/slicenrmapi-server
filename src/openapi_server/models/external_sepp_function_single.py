# coding: utf-8

from __future__ import annotations
from datetime import date, datetime  # noqa: F401

import re  # noqa: F401
from typing import Any, Dict, List, Optional  # noqa: F401

from pydantic import AnyUrl, BaseModel, EmailStr, Field, validator  # noqa: F401
from openapi_server.models.external_sepp_function_single_all_of_attributes import ExternalSeppFunctionSingleAllOfAttributes
from openapi_server.models.vs_data_container_single import VsDataContainerSingle


class ExternalSeppFunctionSingle(BaseModel):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.

    ExternalSeppFunctionSingle - a model defined in OpenAPI

        id: The id of this ExternalSeppFunctionSingle.
        object_class: The object_class of this ExternalSeppFunctionSingle [Optional].
        object_instance: The object_instance of this ExternalSeppFunctionSingle [Optional].
        vs_data_container: The vs_data_container of this ExternalSeppFunctionSingle [Optional].
        attributes: The attributes of this ExternalSeppFunctionSingle [Optional].
    """

    id: str = Field(alias="id")
    object_class: Optional[str] = Field(alias="objectClass", default=None)
    object_instance: Optional[str] = Field(alias="objectInstance", default=None)
    vs_data_container: Optional[List[VsDataContainerSingle]] = Field(alias="VsDataContainer", default=None)
    attributes: Optional[ExternalSeppFunctionSingleAllOfAttributes] = Field(alias="attributes", default=None)

ExternalSeppFunctionSingle.update_forward_refs()
